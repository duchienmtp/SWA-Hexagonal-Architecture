...
public class OrderItem {
    ...
    public void validateItem() {
        if (quantity <= 0) {
            throw new NotificationDomainException("Quantity must be positive");
        }
        if (!price.isGreaterThanZero()) {
            throw new NotificationDomainException("Price must be positive");
        }
        if (!subTotal.isGreaterThanZero()) {
            throw new NotificationDomainException("SubTotal must be positive");
        }
    }
