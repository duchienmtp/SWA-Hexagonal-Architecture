...
public class OrderDataAccessMapper {
    ...
    public OrderJpaEntity toJpaEntity(Order order) {
        return OrderJpaEntity.builder()
                ...
                .price(order.getPrice().getAmount() != null ? order.getPrice().getAmount() : null)
                .failureMessages(order.getFailureMessages() != null ? order.getFailureMessages() : null)
                .createdDate(LocalDateTime.now())
                .items(order.getItems() != null ? order.getItems().stream()
                        .<OrderItemJpaEntity>map(item -> OrderItemDataAccessMapper.toJpaEntity(item,
                                OrderJpaEntity.builder().id(item.getOrderId().getValue()).build()))
